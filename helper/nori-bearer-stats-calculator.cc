...
NS_LOG_COMPONENT_DEFINE("NoriBearerStatsCalculator");

NS_OBJECT_ENSURE_REGISTERED(NoriBearerStatsCalculator);

NoriBearerStatsCalculator::NoriBearerStatsCalculator()
    : m_firstWrite(true),
...
NoriBearerStatsCalculator::NoriBearerStatsCalculator(std::string protocolType)
    : m_firstWrite(true),
      m_pendingOutput(false)
{
    NS_LOG_FUNCTION(this);
...
TypeId
NoriBearerStatsCalculator::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::NoriBearerStatsCalculator")
            .SetParent<NrBearerStatsBase>()
            .AddConstructor<NoriBearerStatsCalculator>()
            .SetGroupName("nr")
            .AddAttribute("StartTime",
                          "Start time of the on going epoch.",
                          TimeValue(Seconds(0.)),
                          MakeTimeAccessor(&NoriBearerStatsCalculator::SetStartTime,
                                           &NoriBearerStatsCalculator::GetStartTime),
                          MakeTimeChecker())
            .AddAttribute("EpochDuration",
                          "Epoch duration.",
                          TimeValue(Seconds(0.25)),
                          MakeTimeAccessor(&NoriBearerStatsCalculator::GetEpoch,
                                           &NoriBearerStatsCalculator::SetEpoch),
                          MakeTimeChecker())
            .AddAttribute("DlRlcOutputFilename",
                          "Name of the file where the downlink results will be saved.",
                          StringValue("NrDlRlcStatsE2E.txt"),
                          MakeStringAccessor(&NoriBearerStatsCalculator::m_dlRlcOutputFilename),
                          MakeStringChecker())
            .AddAttribute("UlRlcOutputFilename",
                          "Name of the file where the uplink results will be saved.",
                          StringValue("NrUlRlcStatsE2E.txt"),
                          MakeStringAccessor(&NoriBearerStatsCalculator::m_ulRlcOutputFilename),
                          MakeStringChecker())
            .AddAttribute("DlPdcpOutputFilename",
                          "Name of the file where the downlink results will be saved.",
                          StringValue("NrDlPdcpStatsE2E.txt"),
                          MakeStringAccessor(&NoriBearerStatsCalculator::m_dlPdcpOutputFilename),
                          MakeStringChecker())
            .AddAttribute("UlPdcpOutputFilename",
                          "Name of the file where the uplink results will be saved.",
                          StringValue("NrUlPdcpStatsE2E.txt"),
                          MakeStringAccessor(&NoriBearerStatsCalculator::m_ulPdcpOutputFilename),
                          MakeStringChecker());
    return tid;
}

void
NoriBearerStatsCalculator::DoDispose()
{
    NS_LOG_FUNCTION(this);
    if (m_pendingOutput)
    {
...
void
NoriBearerStatsCalculator::SetStartTime(Time t)
{
    m_startTime = t;
    // RescheduleEndEpoch ();
}
...
Time
NoriBearerStatsCalculator::GetStartTime() const
{
    return m_startTime;
}

void
NoriBearerStatsCalculator::SetEpoch(Time e)
{
    m_epochDuration = e;
    // RescheduleEndEpoch ();
}
...
Time
NoriBearerStatsCalculator::GetEpoch() const
{
    return m_epochDuration;
}

void
NoriBearerStatsCalculator::UlTxPdu(uint16_t cellId,
                                 uint64_t imsi,
                                 uint16_t rnti,
                                 uint8_t lcid,
                                 uint32_t packetSize)
...
void
NoriBearerStatsCalculator::DlTxPdu(uint16_t cellId,
                                 uint64_t imsi,
                                 uint16_t rnti,
                                 uint8_t lcid,
                                 uint32_t packetSize)
...
void
NoriBearerStatsCalculator::UlRxPdu(uint16_t cellId,
                                 uint64_t imsi,
                                 uint16_t rnti,
                                 uint8_t lcid,
                                 uint32_t packetSize,
...
void
NoriBearerStatsCalculator::DlRxPdu(uint16_t cellId,
                                 uint64_t imsi,
                                 uint16_t rnti,
                                 uint8_t lcid,
                                 uint32_t packetSize,
...
void
NoriBearerStatsCalculator::ShowResults()
{
...
void
NoriBearerStatsCalculator::WriteUlResults(std::ofstream& outFile)
{
    NS_LOG_FUNCTION(this);
...
void
NoriBearerStatsCalculator::WriteDlResults(std::ofstream& outFile)
{
    NS_LOG_FUNCTION(this);
...
void
NoriBearerStatsCalculator::ResetResults()
{
    NS_LOG_FUNCTION(this);
...
void
NoriBearerStatsCalculator::ResetResultsForImsiLcid (uint64_t imsi, uint16_t lcid)
{
  NS_LOG_FUNCTION (this);
...
void
NoriBearerStatsCalculator::RescheduleEndEpoch()
{
    NS_LOG_FUNCTION(this);
    ...
    m_endEpochEvent = Simulator::Schedule(m_startTime + m_epochDuration,
                                          &NoriBearerStatsCalculator::EndEpoch,
                                          this);
}
...
NoriBearerStatsCalculator::EndEpoch()
{
    ...
    m_endEpochEvent =
        Simulator::Schedule(m_epochDuration, &NoriBearerStatsCalculator::EndEpoch, this);
}

uint32_t
NoriBearerStatsCalculator::GetUlTxPackets(uint64_t imsi, uint8_t lcid)
{
...
uint32_t
NoriBearerStatsCalculator::GetUlRxPackets(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
...
uint64_t
NoriBearerStatsCalculator::GetUlTxData(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_ulTxData[p];
...
uint64_t
NoriBearerStatsCalculator::GetUlRxData(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_ulRxData[p];
...
double
NoriBearerStatsCalculator::GetUlDelay(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    Uint64StatsMap::iterator it = m_ulDelay.find(p);
...
std::vector<double>
NoriBearerStatsCalculator::GetUlDelayStats(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
...
std::vector<double>
NoriBearerStatsCalculator::GetUlPduSizeStats(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    std::vector<double> stats;
...
uint32_t
NoriBearerStatsCalculator::GetDlTxPackets(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
...
uint32_t
NoriBearerStatsCalculator::GetDlRxPackets(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
...
uint64_t
NoriBearerStatsCalculator::GetDlTxData(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_dlTxData[p];
...
uint64_t
NoriBearerStatsCalculator::GetDlRxData(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_dlRxData[p];
...
uint32_t
NoriBearerStatsCalculator::GetUlCellId(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_ulCellId[p];
...
uint32_t
NoriBearerStatsCalculator::GetDlCellId(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    return m_dlCellId[p];
...
double
NoriBearerStatsCalculator::GetDlDelay(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    Uint64StatsMap::iterator it = m_dlDelay.find(p);
...
std::vector<double>
NoriBearerStatsCalculator::GetDlDelayStats(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    std::vector<double> stats;
...
std::vector<double>
NoriBearerStatsCalculator::GetDlPduSizeStats(uint64_t imsi, uint8_t lcid)
{
    NS_LOG_FUNCTION(this << imsi << (uint16_t)lcid);
    ImsiLcidPair_t p(imsi, lcid);
    std::vector<double> stats;
...
std::string
NoriBearerStatsCalculator::GetUlOutputFilename()
{
...
std::string
NoriBearerStatsCalculator::GetDlOutputFilename()
{
    if (m_protocolType == "RLC")
    {
