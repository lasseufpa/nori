...
NS_LOG_COMPONENT_DEFINE("NoriLteRlcAm");
    ...
    static TypeId tid =
        TypeId("ns3::LteRlcAm")
            ...
            .AddAttribute(
                ...
                TimeValue(MilliSeconds(20)),
                MakeTimeAccessor(&NoriLteRlcAm::m_rbsTimerValue),
                MakeTimeChecker())
            .AddAttribute("TxOpportunityForRetxAlwaysBigEnough",
                          ...
                          BooleanValue(false),
                          MakeBooleanAccessor(&NoriLteRlcAm::m_txOpportunityForRetxAlwaysBigEnough),
                          MakeBooleanChecker())
            .AddAttribute("MaxTxBufferSize",
                          "Maximum Size of the Transmission Buffer (in Bytes).  If zero is "
                          "configured, the buffer is unlimited.",
                          UintegerValue(10 * 1024),
                          MakeUintegerAccessor(&NoriLteRlcAm::m_maxTxBufferSize),
                          MakeUintegerChecker<uint32_t>());
    return tid;
}

void
NoriLteRlcAm::DoDispose()
{
    NS_LOG_FUNCTION(this);
...
void
NoriLteRlcAm::DoTransmitPdcpPdu(Ptr<Packet> p)
{
    ...
    m_rbsTimer.Cancel();
    m_rbsTimer = Simulator::Schedule(m_rbsTimerValue, &NoriLteRlcAm::ExpireRbsTimer, this);
}
...
void
NoriLteRlcAm::DoNotifyTxOpportunity(LteMacSapUser::TxOpportunityParameters txOpParams)
{
    ...
    if (m_statusPduRequested && !m_statusProhibitTimer.IsRunning())
    {
        ...
        m_statusProhibitTimer = Simulator::Schedule(m_statusProhibitTimerValue,
                                                    &NoriLteRlcAm::ExpireStatusProhibitTimer,
                                                    this);
        return;
    }
                        ...
                        if (!m_pollRetransmitTimer.IsRunning())
                        {
                            ...
                            m_pollRetransmitTimer =
                                Simulator::Schedule(m_pollRetransmitTimerValue,
                                                    &NoriLteRlcAm::ExpirePollRetransmitTimer,
                                                    this);
                        }
                        else
                        {
...
void
NoriLteRlcAm::DoReceivePdu(LteMacSapUser::ReceivePduParameters rxPduParams)
{
        ...
        if (!m_reorderingTimer.IsRunning())
        {
            ...
            if (m_vrH > m_vrR)
            {
                ...
                m_reorderingTimer = Simulator::Schedule(m_reorderingTimerValue,
                                                        &NoriLteRlcAm::ExpireReorderingTimer,
                                                        this);
                m_vrX = m_vrH;
...
bool
NoriLteRlcAm::IsInsideReceivingWindow(SequenceNumber10 seqNumber)
{
...
void
NoriLteRlcAm::ReassembleAndDeliver(Ptr<Packet> packet)
{
    LteRlcAmHeader rlcAmHeader;
    RlcTag rlcTag;
...
void
NoriLteRlcAm::DoReportBufferStatus()
{
    NS_LOG_FUNCTION(this);
...
NoriLteRlcAm::ExpireReorderingTimer()
{
    ...
    if (m_vrH > m_vrMs)
    {
        NS_LOG_LOGIC("Start reordering timer");
        m_reorderingTimer =
            Simulator::Schedule(m_reorderingTimerValue, &NoriLteRlcAm::ExpireReorderingTimer, this);
        m_vrX = m_vrH;
        NS_LOG_LOGIC("New VR(MS) = " << m_vrMs);
    }
